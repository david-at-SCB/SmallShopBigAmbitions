...
public class A_TraceableTExample
{
    ...
    private static IO<Option<EnrichedUserProfile>> EnrichUserAsyncVersion(string user, Microsoft.Extensions.Logging.ILogger logger)
    {
        ...
        return from p in tracedProfile.RunTraceable()
               from b in tracedBadge.RunTraceable()
               from e in tracedExtra.RunTraceable()
               let profileOpt = Flatten<string>(p) // Flatten the Fin<Option<T>> to Option<T>. T is string in this case.
               let badgeOpt = Flatten<string>(b) // Flatten the Fin<Option<T>> to Option<T>. T is string in this case.
               let extraOpt = Flatten(e) // Flatten the Fin<Option<T>> to Option<T>. T is string in this case, so Flatten<string>(e) is the entire type signature.
